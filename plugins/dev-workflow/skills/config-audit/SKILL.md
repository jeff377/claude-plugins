...
#### 生命週期：這個 skill 描述的工作還會不會再發生？

上面那些檢查的是**觸發面**（叫不叫得到、會不會叫錯）。它們回答不了另一個問題：
**這份工作本身是不是已經結束了。** 兩者必須分開問 —— 一支 description 寫得完美、
歸屬層正確、引用全部有效的 skill，仍然可能是為一次性任務寫的，而那個階段早就過去。

依序問三題：

1. **這個 skill 描述的工作還會再發生嗎？**
   - 會週期性再發生（發版、發文、新增模組、每加一個平台頭就重走一次）→ **保留**
   - 一次性完成且成果已落地（某次遷移、某次打包、某個 app 建起來了）→ **候選清理**
   - 分不出來 → **需使用者判斷**，不要猜
2. **內容是「程序知識」還是「踩雷誌」？** 踩雷誌即使那次任務結束仍有價值（下次踩同一個雷），
   但未必該留在 skill —— 見下方降級對照。這種標 **降級**，不是刪除。
3. **description 裡有沒有自己寫下的劃界句？**「X 已非首選」「沒指名時請用 Y」這類句子
   多半是上一輪健檢留下的緩刑，**它本身就是候選清理的直接線索** —— 去查那次緩刑的理由還在不在。

**判定證據在 repo 裡，不在 `SKILL.md` 裡。** skill 內文永遠會說自己有用；要看的是這套做法
現在還有幾個消費者。掃**所有**工作 repo，不只當前這個：

```bash
# 這個 pattern / 技術棧還有幾個消費者？
grep -rl "<該 skill 的標誌性識別字>" <repos 根> --include="*.<ext>" | grep -v "/bin/\|/obj/"
# 對應的技術棧是不是已經被移除過？
git log --oneline -i --grep="<技術棧名>" | head
```

零消費者、或唯一消費者的那次工作已 commit 完成，就是「不會再發生」的實證。

**降級目標對照：**

| skill 的內容性質 | 降級到 |
|---|---|
| 某 repo 專屬的踩雷（症狀 / 根因 / 正解） | 該 repo 的踩雷誌目錄（`docs/repo-ops/gotchas/` 之類）—— 進版控、review 得到、隨程式碼演進 |
| 單一專案的建置 / 打包程序 | 該專案的 `README.md` 或 `docs/` |
| 只有一兩行的結論（「不要改回 X，因為…」） | 直接寫進**原始碼註解**，緊貼被保護的那行 |

分級落在報告的 **P2**（降級與清理都是搬家或移除，不是正在誤導 agent 的 P0 / P1）。
標成「需使用者判斷」的，**必須寫出決定性問題是什麼**（「未來還會不會再有第二個 X？」）
並附上兩邊的證據 —— 只寫「看情況」等於把判斷丟回去卻沒附上判斷所需的資訊。

> **實例：** 某支為某 UI 框架寫的 scaffold skill，該框架六週前已被一個明寫「移除 X」的
> commit 從 repo 整個拿掉，全部工作 repo 對它的建置屬性零命中。上一輪健檢確實掃到它
> 與另一支 skill 觸發重疊，依「先修描述、不急著刪」加了劃界句就收工 —— **而修描述
> 修不掉一個已經消失的使用場景**，它因此又留了六週。
...
| 只查觸發面、不問工作是否已結束 | description 完美、引用全有效的 skill，照樣可能是為一次性任務寫的 |
| 把踩雷誌型的 skill 直接刪掉 | 雷還在，下次會重踩 —— 該降級到該 repo 的踩雷誌，不是移除 |
